...
#define REAL_ETH_NAME	"eth0"
static int hiwifi_eth_led_status = 1;
static struct proc_dir_entry *hiwifi_proc_dir;
static struct proc_dir_entry *hiwifi_eth_led;

static int hiwifi_eth_led_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	return sprintf(page, "%d\n", hiwifi_eth_led_status);
}

static int hiwifi_eth_led_write(struct file *file, const char *buf,
								unsigned long count, void *data)
{
	...
	hiwifi_eth_led_status = val;

	dev = dev_get_by_name(&init_net, REAL_ETH_NAME);
	...
	etc_eth_leds_status(et->etc, hiwifi_eth_led_status);

	dev_put(dev);
	return count;
}

static void __init et_create_leds_proc(void)
{
	hiwifi_proc_dir = proc_mkdir("hiwifi", NULL);
	if (!hiwifi_proc_dir)
	{
		printk(KERN_ERR "hiwifi proc dir create failed\n");
		return;
	}

	hiwifi_eth_led = create_proc_entry("eth_led", 0644, hiwifi_proc_dir);
	if (!hiwifi_eth_led)
	{
		printk(KERN_ERR "hiwifi_eth_led proc entry create failed\n");

		remove_proc_entry("hiwifi", NULL);
		return;
	}

	hiwifi_eth_led->write_proc = hiwifi_eth_led_write;
	hiwifi_eth_led->read_proc = hiwifi_eth_led_read;	
}

static void __exit et_remove_leds_proc(void)
{
	if (hiwifi_eth_led != NULL)
	{
		if (hiwifi_proc_dir)
		{
			remove_proc_entry("eth_led", hiwifi_proc_dir);
		} 
	}
...
et_open(struct net_device *dev)
{
	...
	/*
	 * open promisc at init to make sure gmac can recv all frame
	 */
	et->etc->promisc = TRUE;
...
et_get_settings(struct net_device *dev, struct ethtool_cmd *ecmd)
{
	...
	ecmd->advertising |= (et->etc->advertise & ADVERTISE_10HALF) ?
	        ADVERTISED_10baseT_Half : 0;
	ecmd->advertising |= (et->etc->advertise & ADVERTISE_10FULL) ?
	        ADVERTISED_10baseT_Full : 0;
	ecmd->advertising |= (et->etc->advertise & ADVERTISE_100HALF) ?
	        ADVERTISED_100baseT_Half : 0;
	ecmd->advertising |= (et->etc->advertise & ADVERTISE_100FULL) ?
	        ADVERTISED_100baseT_Full : 0;
	ecmd->advertising |= (et->etc->advertise2 & ADVERTISE_1000FULL) ?
	        ADVERTISED_1000baseT_Full : 0;
	ecmd->advertising |= (et->etc->advertise2 & ADVERTISE_1000HALF) ?
	...
	case SIOCGMIIREG:
		{
			...
			uint phy_val;
			struct mii_ioctl_data *data = if_mii(ifr);
			phy_val = etc_get_phy_status(et->etc);
			if(phy_val & 0x10)
				data->val_out = 1 << 2;
			else
				data->val_out = 0;
