...
uint etc_get_phy_status(etc_info_t *etc)
{
#ifdef ETROBO 	
	if (etc->robo) {
		uint16 val;
		...
		robo->ops->read_reg(etc->robo, 0x1, 0x0, &val, 2);
...
void etc_eth_leds_status(etc_info_t *etc, uint on)
{
#ifdef ETROBO
	robo_info_t *robo = (robo_info_t *)etc->robo;
	if (robo)
	{
		bcm_robo_leds_status(robo, on);
	}	
#endif
}
