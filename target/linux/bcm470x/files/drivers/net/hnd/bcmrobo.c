...
#define REG_LED_MAP_EN	0x16	/* LED Map enabel register */
#define REG_LED_MODE_MAP0 0x18	/* LED Mode Map 0 regiseter */
#define REG_LED_MODE_MAP1 0x1a	/* LED Mode Map 1 register */
...
bcm_robo_enable_switch(robo_info_t *robo)
{
	...
	if (ROBO_IS_BCM5301X(robo->devid)) {
		val16 = 0x1F;
		robo->ops->write_reg(robo, PAGE_CTRL, REG_LED_MAP_EN, &val16, sizeof(val16));
	}
 ...
 * open/close eth leds 
 */
void bcm_robo_leds_status(robo_info_t *robo, uint32 status)
{
	uint16 val16;
	...
	if (ROBO_IS_BCM5301X(robo->devid))
	{
		if (status)
		{
			 ...
			 */
			val16 = 0x1FF;
			robo->ops->write_reg(robo, PAGE_CTRL, REG_LED_MODE_MAP0, &val16, sizeof(val16));
			robo->ops->write_reg(robo, PAGE_CTRL, REG_LED_MODE_MAP1, &val16, sizeof(val16));
		}
		else
		{
			/*
			 * disable maps, leds off
			 */
			val16 = 0x0;
			robo->ops->write_reg(robo, PAGE_CTRL, REG_LED_MODE_MAP0, &val16, sizeof(val16));
			robo->ops->write_reg(robo, PAGE_CTRL, REG_LED_MODE_MAP1, &val16, sizeof(val16));
