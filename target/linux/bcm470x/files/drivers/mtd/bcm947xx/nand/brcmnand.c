...
brcmnand_scan(struct mtd_info *mtd)
{
	...
	do {
		...
		ret = brcmnand_hw_ecc_layout(brcmnand);

		if (ret && chip->page_shift > 9 && brcmnand->sector_size_shift < 10)
			brcmnand->sector_size_shift = 10;
		else if (ret)
			brcmnand->ecc_level--;
	}while( ret && brcmnand->ecc_level > 0 );

	BUG_ON(brcmnand->ecc_level == 0);

	if( (brcmnand->sector_size_shift  > 9 ) != (sector_1k==1) ) {
	...
	if( brcmnand->ecc_level != nfl->ecclevel) {
		printk(KERN_WARNING "%s: ECC level adjusted from %u to %u\n",
			DRV_NAME, nfl->ecclevel, brcmnand->ecc_level );
		nfl->ecclevel = brcmnand->ecc_level;
	}
	...
	reg = R_REG(osh, &nc->acc_control_cs1);
	if (sector_1k)
	{
		reg |= NANDAC_CS0_SECTOR_SIZE_1K;
		W_REG(osh, &nc->acc_control_cs1, reg);
	}

	reg = R_REG(osh, &nc->acc_control_cs0);
	reg &= ~NANDAC_CS0_ECC_LEVEL_MASK;
	reg |= ((nfl->ecclevel << NANDAC_CS0_ECC_LEVEL_SHIFT) & NANDAC_CS0_ECC_LEVEL_MASK);
	W_REG(osh, &nc->acc_control_cs0, reg);
	...
	printk(KERN_ERR "reg xxx: 0x%x\n", R_REG(osh, &nc->acc_control_cs1));
#endif
