...
static int nandc_hw_ecc_layout( struct nandc_ctrl * ctrl )
{
	...
	if( (ctrl->sec_per_page_shift != 0 && ecc_per_sec >= oob_per_sec) ||
		...
		pr_debug("%s: ERROR: ECC level %d too high, "
...
static irqreturn_t nandc_isr(int irq, void *dev_id)
{
	...
	pr_debug("%s:start irqoff=%d irq_reg=%x en=%d cmd=%#x\n",
		__func__, irq_off,
...
static int __init nandc_regs_map( struct nandc_ctrl * ctrl )
{
	...
	if( res != 0) {
		pr_debug("%s: reg resource failure\n", __func__);
		return res ;
		}

	res = request_resource( &iomem_resource, &nandc_idm_regs[ 0 ] );
	if( res != 0) {
		pr_debug("%s: idm resource failure\n", __func__);
		return res ;
		}
	...
	if( IS_ERR_OR_NULL(ctrl->idm_base) ) {
		...
		pr_debug("%s: error mapping wrapper\n", __func__);
...
static int __devinit nandc_probe(struct platform_device *pdev )
{
	...
	if( res ) {
		pr_debug("%s: regs_map failed\n", __func__);
		goto probe_error ;
		}

	res = nandc_idm_init( ctrl );
	if( res ) {
		pr_debug("%s: idm_init failed\n", __func__);
		goto probe_error ;
		}

	res = nandc_ctrl_init( ctrl );
	if( res ) {
		pr_debug("%s: ctrl_init failed\n", __func__);
		goto probe_error ;
		}
	...
	if( res > 0 ) {
		pr_debug("%s: registering MTD partitions\n",
			__func__);
...
#else
	{
	    pr_debug("%s: registering the entire device MTD\n",
			__func__);
	    if( (res = add_mtd_device( &ctrl->mtd ))) {
		pr_debug("%s: failed to register\n", __func__);
		goto probe_error ;
		}
	}
#endif
