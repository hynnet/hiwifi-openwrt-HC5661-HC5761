...
spiflash_init(si_t *sih)
{
	...
	spiflash.write = spiflash_commit;
...
static uchar *spiflash_block_buf = NULL;
...
spiflash_commit(hndsflash_t *spifl, uint offset, uint len, const uchar *buf)
{
	...
	if (!spiflash_block_buf)
	{
		/* Allocate a block of mem */
		if (!(spiflash_block_buf = MALLOC(osh, blocksize)))
		{
			printk(KERN_ERR "alloc spi flash block buffer error\n");
			return -1;
		}
	}

	block = spiflash_block_buf;
	...
	while (len) {
		...
		pad = offset - cur_offset;

		if (pad == (uint)0)
		{
			/*
			 * offset is align to block size
			 * write buf to flash 
			 */
			cur_ptr = buf;
			cur_length = cur_retlen;
			while (cur_length > 0) {
				if ((bytes = spiflash_write(spifl,
				                          ...
				                          (uchar *)cur_ptr)) < 0) {
					ret = bytes;
					printk(KERN_ERR "spiflash_write error\n");
					goto done;
				}
				cur_offset += bytes;
				cur_length -= bytes;
		...
		{
			/*
			 * offset is not align to block size, need to align
			 * read align data and copy data to block buf and write to flash
			 */
			cur_length = pad + len;
