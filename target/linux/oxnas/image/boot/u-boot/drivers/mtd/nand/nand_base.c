	...
	u_char *oob_buf,  struct nand_oobinfo *oobsel, int cached)
{
	...
	/* No ecc, write all */
	case NAND_ECC_NONE:
		printk (KERN_WARNING "Writing data without ECC not recommended\n");
...
/**
 * nand_read_up - [MTD Interface] Read data with single bad block skip handling
 * @mtd:        MTD device structure
 * @from:       offset to read from, must be a multiple of the erasesize size
 * @len:        number of bytes to read
 * @buf:        the databuffer to put data
 *
*/
int nand_read_up (struct mtd_info *mtd, loff_t from, size_t len, u_char * buf)
{
	struct nand_chip *this = mtd->priv;
	...
	if (from % pagesize != 0)
	{
		printf("Offset not a multiple of pagesize size (%d)\n", pagesize);
		return -EINVAL;
	}
	...
	while (len > 0)
	{
		size_t lentoread = len > (erasesize - off) ? (erasesize - off) : len;
		size_t retsize;

		while (nand_block_isbad(mtd, from - off))
		{
			totalbadblocks++;
			from += erasesize;
		}
		
		int ret = nand_read_ecc (mtd, (loff_t) from, lentoread, &retsize, buf, NULL, NULL);
...
{
	struct nand_chip *this = mtd->priv;
	int pagedata = mtd->oobblock;
	int erasesize = mtd->erasesize;
	// int pagesperblock = erasesize / (blocksize + mtd->oobsize);
	int pagesize = 1 << this->page_shift;
	...
	while (len > 0)
	{
		...
		for ( page_cnt = 0; page_cnt < pagesperblock / 2; page_cnt++){
			size_t lentoread = len > pagedata ? pagedata : len;
			badpage = 0;
		
		readpage:
			//printf("read from %x, len %x\n", (int)(from + badpage * pagesize +  skip * erasesize), lentoread);
			ret = nand_read_ecc (mtd, (loff_t)(from + badpage * pagesize +  skip * erasesize), lentoread, &retsize, buf, NULL, NULL);
			//printf("ret %d\n", ret);
			if (ret < 0)
			{
				totalbadpages++;
				if (++badpage > CONFIG_PAGE_REPLICATION - 1)
				{
			...
			{
				from += pagesize * CONFIG_PAGE_REPLICATION;
				len -= retsize;
				buf += retsize;
				if (len <=0 )
					break;
	...
	if (totalbadpages > 0 || totalbadblocks > 0)
	{
		printf("%d bad pages, %d bad blocks\n", totalbadpages, totalbadblocks);
	}
	return 0;
}
 ...
 *
*/
int nand_write_mlc (struct mtd_info *mtd, loff_t from, size_t len, size_t * retlen, u_char * buf)
{
	struct nand_chip *this = mtd->priv;
	int pagedata = mtd->oobblock;
	...
	const loff_t blockskip = (CONFIG_FLASH_KERNEL_SPACE / pagedata + 1) * CONFIG_PAGE_REPLICATION * CONFIG_BLOCK_REPLICATION / pagesperblock;
	int badblocks = 0;
	loff_t nblock, npage, skip;
	size_t lentowrite;

	printf("Write to flash with page replication of %d and block replication of %d\n", CONFIG_PAGE_REPLICATION, CONFIG_BLOCK_REPLICATION);
	if (from % erasesize != 0)
	{
	...
	while (len > 0)
	{
		...
		for (nblock = 0; nblock < CONFIG_BLOCK_REPLICATION; nblock++)
		{
			...
			{
				size_t retsize;
				//printf("write to address %lx, skip %lx, page %lx, block %lx\n", (long)(from + npage * pagesize + skip * erasesize), (long)(skip * pagesize), (long)npage, (long)nblock);
				int ret = nand_write_ecc (mtd, (loff_t)(from + npage * pagesize + skip * erasesize), lentowrite, &retsize, buf, NULL, NULL);
				if (ret < 0)
				{
					printf("Bad page for copy %lx of block %lx\n", (long)npage, (long)nblock);
				}
			}
			skip += blockskip;
		}
			   ...
			   size_t * retlen, const u_char * buf, u_char * eccbuf, struct nand_oobinfo *oobsel)
{
	...
	*retlen = 0;
	...
	if ((to + len) > mtd->size) {
		MTDDEBUG (MTD_DEBUG_LEVEL0,
		          "nand_write_ecc: Write past end of page\n");
		return -EINVAL;
	}

	/* reject writes, which are not page aligned */
	if (NOTALIGNED (to) || NOTALIGNED(len)) {
		printk (KERN_NOTICE "nand_write_ecc: Not page aligned data\n");
		return -EINVAL;
	}
...
/*			BUG(); */
	...
	*/
	...
	case NAND_ECC_NONE:
		printk (KERN_WARNING "NAND_ECC_NONE: Not recommended !!\n");
