...
static void ProgStart(void);
/*
static prog_cmd_t ProgCmdMode = S_LISTEN;
*/
#endif
...
#if (CONFIG_COMMANDS & CFG_CMD_PROG)
static void
ProgHandler (uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	int chunk;
	uchar *outpkt;
	char tempBuf[PROG_MAXLINE], *cmd_string, *val, *name;
	prog_hdr_t *progh = (prog_hdr_t *)pkt;
	IP_t *iph = (IP_t *)(pkt - IP_HDR_SIZE);
	Ethernet_t *ethhdr = (Ethernet_t *)(pkt - IP_HDR_SIZE - ETHER_HDR_SIZE);

	if (dest != PROG_DEF_PORT || !len)
		return;         /* not for us */

	outpkt = (uchar *) NetTxPacket + NetEthHdrSize() + IP_HDR_SIZE;
	switch(ntohs(progh->code)) {
		case S_REQUEST:
			cmd_string = strncpy(&tempBuf[0], getenv(progh->data), PROG_MAXLINE);
			cmd_string[PROG_MAXLINE-1] = '\0';
			break;
		case S_SETENV:
			if ((val = strchr(progh->data, '=')) == NULL)
				return;
			name = progh->data;
			*val++ = '\0';
			if (getenv(name) == NULL)
			...
			setenv(name, val);
			saveenv();
			cmd_string = strncpy(&tempBuf[0], getenv(name), PROG_MAXLINE);
			cmd_string[PROG_MAXLINE-1] = '\0';
			break;
		case S_QUIT:
			NetState = NETLOOP_SUCCESS;
	...
	chunk = strlen(cmd_string) + 1;
	progh = (prog_hdr_t *)outpkt;
	progh->tot_len = htons(chunk + sizeof(prog_hdr_t));
	progh->code = htons(S_REPLY);
	memcpy(progh->data, cmd_string, chunk);

	NetSendUDPPacket(ethhdr->et_src, iph->ip_src, src, PROG_DEF_PORT, chunk + sizeof(prog_hdr_t));
	return;
}
...
//			return;
			if (!NetOurSubnetMask ||
				tmp != ((NetOurIP & NetOurSubnetMask) | (~NetOurSubnetMask)))
				return;
		}
		/*
