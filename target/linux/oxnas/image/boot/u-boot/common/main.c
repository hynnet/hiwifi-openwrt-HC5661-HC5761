...
#if (CONFIG_COMMANDS & CFG_CMD_PROG)
#define GPIO_NUM_PER_REG       32
static __inline__ u32 read_gpio(u32 gpio_num)
{
	if (gpio_num >= GPIO_NUM_PER_REG) {
		return ((*(volatile u32*)GPIO_2_IN) & (1UL << (gpio_num - GPIO_NUM_PER_REG))) >> (gpio_num - GPIO_NUM_PER_REG);
	} else {
		return ((*(volatile u32*)GPIO_1_IN) & (1UL << gpio_num)) >> gpio_num;
	}
}
#endif
...
#ifdef SDK_BUILD_FLASH_BOOT
#if (USE_NAND == 1)
/* Check upgrade flag on NAND flash */
int check_nand_upgrade(void)
{
	int ret, badcnt=0, badreg=0;
	size_t size;
	nand_info_t *nand;
	off_t from;
	ulong addr = MODE_FLAG_LOCATION;

	nand=&nand_info[nand_curr_device];

	size = 1;

	from = 0x1EC0000; //0x12c0000;
	...
	ret = nand_read(nand, from, &size, (u_char *)addr);

	printf("Reading upgrade flag from NAND address 0x%08x : %x\n", from, ret);
...
void main_loop (void)
{
...
#if (CONFIG_COMMANDS & CFG_CMD_PROG)
	run_command("run lightled", 0);
	if (!read_gpio(CONFIG_PROG_BTN_GPIO)) {
		run_command("run extinguishled", 0);
		run_command("prog", 0);
	}
#endif
	...
	if (SET_LINUX_MEM_SIZE) {
		DECLARE_GLOBAL_DATA_PTR;

		printf("Setting Linux mem= boot arg value\n");
		...
		switch (megabytes) {
			...
			case 256:
				...
				break;
			case 512:
				cmd_string = strcat(cmd_string, " mem=512M");
				break;
			default:
	...
	ide_init();
	run_command("ide read " STRINGIFY(MODE_FLAG_LOCATION) " 120 1", 0);
...
#if (USE_NAND == 1)
	int ret = 0;
	ret=check_nand_upgrade();
#endif

	char upgrade_mode = *(volatile char*)(MODE_FLAG_LOCATION + 0);	

#endif //SDK_BUILD_FLASH_BOOT
	...
	int i;
	int otp_set = 0;
	for (i=0; i< 6; i++) otp_set+=read_otp_byte(OTP_MAC+i); 

	/* remove mac address ready to be replaced from OTP */
	if(otp_set) remove_string_from_bootargs("mac_adr=");
#endif // USE_OTP_MAC
...
#if (NAS_VERSION == 820)
		char sram_data = *(volatile char*)(SYS_CTRL_SCRATCHWORD2);        
#else        
...
#if (NAS_VERSION == 820)
            // Clear the power up flag now
            *(volatile char*)(SYS_CTRL_SCRATCHWORD2) = 0;
#endif            
		} else {
...
#if (NAS_VERSION == 820)
		unsigned int ms_timeCount = *(volatile unsigned int*)(SYS_CTRL_SCRATCHWORD3);        
#else        
...
#if (USE_OTP_MAC == 1)
	/* put mac address back from OTP */
	if (otp_set) sprintf( cmd_string, "%s mac_adr=0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x",
		cmd_string,
	...
	if (upgrade_mode == UPGRADE_MAGIC) {
        ...
        parse_string_outer("set loadf ide read " STRINGIFY(UPGRADE_FS_LOCATION) " 4122 200", FLAG_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP);
        
        /* Script for loading 2MB of upgrade kernel image from hidden sectors */
        parse_string_outer("set loadk ide read " STRINGIFY(UPGRADE_KERNEL_LOCATION) " 2122 1000", FLAG_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP);
        ...
        parse_string_outer("set boot bootm " STRINGIFY(UPGRADE_KERNEL_LOCATION) " " STRINGIFY(UPGRADE_FS_LOCATION), FLAG_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP);
	...
	/* branch off inot recovery or upadate */
        if (upgrade_mode == UPGRADE_MAGIC) {
		/* Script for loading 6MB of upgrade kernel image from hidden blocks */
		parse_string_outer("set loadk nbootup " XSTRINGIFY(UPGRADE_KERNEL_LOCATION) " 0 0x1EC1000", FLAG_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP);

		/* Script for booting Linux kernel image with mkimage-wrapped initrd */
        	parse_string_outer("set boot bootm " XSTRINGIFY(UPGRADE_KERNEL_LOCATION), FLAG_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP);
		...
		/* Set Linux bootargs to use rootfs in initial ramfs */
	        parse_string_outer("set bootargs mem=128M console=ttyS0,115200", FLAG_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP);

		parse_string_outer("run loadk boot || run lightled ", FLAG_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP);
	}
#endif
#endif // SDK_BUILD_FLASH_BOOT
#endif // CONFIG_OXNAS
