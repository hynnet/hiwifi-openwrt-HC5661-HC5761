 ...
 *
 * $Id: ui_bcm947xx.c 371897 2012-11-29 20:19:17Z $
 */
...
void
ui_get_bdinfo_flashdev(char *flashdev)
{
	if (!flashdev)
		return;

	if (soc_knl_dev((void *)sih) == SOC_KNLDEV_NANDFLASH)
		strcpy(flashdev, "nflash1.bdinfo");
	else
		strcpy(flashdev, "flash1.bdinfo");

	return;
}

void
ui_get_backup_flashdev(char *flashdev)
{
	if (!flashdev)
		return;

	if (soc_knl_dev((void *)sih) == SOC_KNLDEV_NANDFLASH)
		strcpy(flashdev, "nflash1.backup");
	else
		strcpy(flashdev, "flash1.backup");
