 ...
 *
 * $Id: eapd.c 394167 2013-04-01 08:35:34Z $
 */
...
{
	int i;
	char ifnames[256], name[IFNAMSIZ];
	char *next;
	...
	eapd_safe_get_conf(ifnames, sizeof(ifnames), "wl_ifnames");
...
eapd_add_interface(eapd_wksp_t *nwksp, char *ifname, eapd_app_mode_t mode, eapd_cb_t **cbp)
{
	...
	*cbp = NULL;
	...
	if (wl_wlif_is_psta(os_name) || find_in_list(ifnames, ifname))
		lanifname = ifname;
	else if ((lanifname = get_ifname_by_wlmac(mac, ifname)) == NULL)
		return FALSE;
...
eapd_wksp_find_cb(eapd_app_t *app, char *wlifname, uint8 *mac)
{
	...
	ifname = (wl_wlif_is_psta(wlifname) ? wlifname : get_ifname_by_wlmac(mac, wlifname));
	if (ifname) {
		cb = app->cb;
		while (cb) {
...
eapd_add_brcm(eapd_wksp_t *nwksp, char *ifname)
{
	...
	strcpy(brcmSocket->ifname, ifname);
...
event_init(eapd_wksp_t *nwksp)
{
	...
	foreach(name, all_ifnames, next) {
		...
		/*
		  * NOTE: Because wl0.1 wl0.2 have same event queue as wl0, we can not
		  * have separate event bitvec just like bsscfg.
		  */
		if (wlunit[unit] != -1) {
