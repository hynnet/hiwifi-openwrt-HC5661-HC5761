 ...
 *
 * $Id: wlu.c 395988 2013-04-10 15:30:09Z $
 */
...
#endif
static cmd_func_t wl_reset_bt_chip;
...
cmd_t wl_cmds[] = {
	...
	"print device path" },
	{ "reset_bt_chip", wl_reset_bt_chip, -1, -1,
		"reset bluetooth chipset by toggling BT_RESET_N\n" },
	...
	{"intfer_params", wl_intfer_params, WLC_GET_VAR, -1,
	"get intfer params \n"
	"\t Usage: wl intfer_params period cnt win dmaate_thld retry_thld glitch_thld txbad_thld"
	},
...
static int
wl_reset_bt_chip(void *wl, cmd_t *cmd, char **argv)
{
	uint32 val;
	uint argc;
	uint32 *int_ptr;
	...
	if (argc == 0) {
		...
		val = htod32(val);
		memcpy(int_ptr, (const void *)&val, sizeof(val));
		return wlu_iovar_set(wl, "reset_bt_chip", buf, sizeof(uint32) *2);
	}
	else
		return -1;
	...
	if (!*++argv) {
		...
		if (phytype == WLC_PHY_TYPE_AC) {
			...
			case NON_WLAN:
				...
				printf("enabled. (mode 1)\n");
				break;
			case WLAN_MANUAL:
				printf("Wireless LAN Interference mitigation ");
				printf("is enabled. (mode 2)\n");
				break;
			case WLAN_AUTO:
				printf("Auto Wireless LAN Interference mitigation is ");
				if (mode & AUTO_ACTIVE)
					printf("enabled and active. (mode 3)\n");
				else
					printf("enabled and not active. (mode 3)\n");
				break;
...
wl_intfer_params(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*argv == NULL) {
		/* get current txdelay params */
		if ((err = wlu_iovar_get(wl, cmd->name, (void *) &param,
			(sizeof(intfer_params_t)))) < 0)
			return (err);
			if (param.version != INTFER_VERSION)
				printf("version[%d] is not match driver(%d)\n",
					INTFER_VERSION, param.version);
			else
				printf("Interference params: period[%d] cnt[%d] win[%d] "
					"dmarate_thld[%d] retry_thld[%d] glitch_thld[%d] "
					"txbad_thld[%d] txnoack_thld[%d]\n",
					param.smplperiod, param.smplcnt, param.smplwin,
					param.dmarate_thld, param.retry_thld, param.glitch_thld,
					param.txbad_thld, param.txnoack_thld);
	}
	else {
		char *endptr;
		/* Validate num of entries */
		...
		argc = 0;
		param.smplperiod = strtol(argv[argc], &endptr, 0);
		argc++;
		param.smplcnt = strtol(argv[argc], &endptr, 0);
		argc++;
		param.smplwin = strtol(argv[argc], &endptr, 0);
		argc++;
		param.dmarate_thld = strtol(argv[argc], &endptr, 0);
		argc++;
		param.retry_thld = strtol(argv[argc], &endptr, 0);
		argc++;
		param.glitch_thld = strtol(argv[argc], &endptr, 0);
		argc++;
		param.txbad_thld = strtol(argv[argc], &endptr, 0);
		argc++;
		param.txnoack_thld = strtol(argv[argc], &endptr, 0);

		/* Set intfer params */
		err = wlu_iovar_set(wl, cmd->name, (void *) &param,
			(sizeof(intfer_params_t)));
	}
	return (err);
}
