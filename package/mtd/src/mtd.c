		...
		if ( (nwritten = write(fd, ptr, nleft)) <= 0) {
			if (nwritten < 0) {
				if (errno == EINTR) {
					...
					try++;
					if (try > RETRY_TIMES) {
						mtd_log_write("Error writing image : %s try %d failed.\n", strerror(errno), RETRY_TIMES);
						return -1;
					} else {
						mtd_log_write("Error writing image : %s (try %d of %d).\n", strerror(errno), try, RETRY_TIMES);
					}
				}
			} else {
...
int mtd_open(const char *mtd, bool block, int rw)
{
	FILE *fp;
	char dev[PATH_MAX];
	int i;
	int ret;
	int flags = rw?(O_RDWR | O_SYNC):(O_RDONLY | O_SYNC);
...
int mtd_check_open(const char *mtd, int rw)
{
	struct mtd_info_user mtdInfo;
	int fd;

	fd = mtd_open(mtd, false, rw);
	if(fd < 0) {
		mtd_log_write("Could not open mtd device: %s\n", mtd);
		return -1;
	}

	if(ioctl(fd, MEMGETINFO, &mtdInfo)) {
		mtd_log_write("Could not get MTD device info from %s\n", mtd);
		close(fd);
		return -1;
	}
...
int mtd_erase_block(int fd, int offset)
{
	...
	while (1) {
		if (ioctl(fd, MEMUNLOCK, &mtdEraseInfo) < 0) {
			if (errno == EINTR) {
				continue;
				...
				try++;
				if (try > RETRY_TIMES) {
					mtd_log_write("Error mem unlock : %s try %d failed.\n", strerror(errno), RETRY_TIMES);
					break;
				} else {
					mtd_log_write("Error mem unblock : %s (try %d of %d).\n", strerror(errno), try, RETRY_TIMES);
				}
			}
		} else {
			break;
	...
	while (1) {
		if (ioctl (fd, MEMERASE, &mtdEraseInfo) < 0) {
			if (errno == EINTR) {
				...
				try++;
				if (try > RETRY_TIMES) {
					mtd_log_write("Error erase block : %s try %d failed.\n", strerror(errno), RETRY_TIMES);
					break;
				} else {
					mtd_log_write("Error erase block : %s (try %d of %d).\n", strerror(errno), try, RETRY_TIMES);
				}
			}
		} else {
				return 0;
...
static int mtd_check(const char *mtd)
{
	...
	do {
		...
		if (!buf) {
			...
			if (!buf) {
				mtd_log_write("Failed to alloc memory.\n");
				mtd_log_exit();
				exit(1);
			}
		}
...
static int
mtd_setrwflag(const char *mtd, const int rwflag)
{
	struct erase_info_user mtdLockInfo;
	char *next = NULL;
	char *str = NULL;
	...
	do {
		...
		if (quiet < 2)
			fprintf(stderr, "setflag %s ...\n", mtd);

		mtdLockInfo.start = 0;
		mtdLockInfo.length = mtdsize;
		ioctl(fd, (rwflag?MEMSETWR : MEMSETRO), &mtdLockInfo);
		close(fd);
		mtd = next;
	} while (next);
...
static int
mtd_verify(int imagefd, const char *mtd)
{
	uint32_t f_md5[4], m_md5[4];
	md5_ctx_t ctx;
	int ret = 0;
	int fd;
	ssize_t imagefd_len = 0;

	if (!quiet)
		mtd_log_write("Verifying %s against %s ...\n", mtd, imagefile);

	if (md5sum_fd(imagefd, f_md5, &imagefd_len)) {
		mtd_log_write("Failed to hash %s.\n", imagefile);
		return -1;
	}
	...
	if (!quiet) {
		mtd_log_write("file size: 0x%x.\n", imagefd_len);
	}

	md5_begin(&ctx);
	do {
		char buf[256];
		int len = (imagefd_len > sizeof(buf)) ? (sizeof(buf)) : (imagefd_len);
	...
	if (!ret)
		...
		if (quiet < 2) {
			...
			fprintf(stderr, "%08x%08x%08x%08x - %s\n", f_md5[0], f_md5[1], f_md5[2], f_md5[3], imagefile);
		}
...
{
	if (quiet < 2)
		mtd_log_write("Writing from %s to %s ... \n", imagefile, mtd);
...
next:
		...
		if (ret < 0) {
			mtd_log_write("Failed to validate the new FIS partition table\n");
			mtd_log_exit();
			exit(1);
		}
		if (ret == 0)
...
resume:
	...
	if (part_offset > 0) {
		mtd_log_write("Seeking on mtd device '%s' to: %u\n", mtd, part_offset);
	...
	for (;;) {
			...
			while (1) {
				...
				if (r < 0) {
					if ((errno == EINTR) || (errno == EAGAIN))
						...
						try++;
						if (try > RETRY_TIMES) {
							mtd_log_write("Error read imagefd : %s try %d failed.\n", strerror(errno), RETRY_TIMES);
							break;
						} else {
							mtd_log_write("Error read imagefd : %s (try %d of %d).\n", strerror(errno), try, RETRY_TIMES);
						}
					}
				} else {
					break;
		...
		if (jffs2file) {
			if (memcmp(buf, JFFS2_EOF, sizeof(JFFS2_EOF) - 1) == 0) {
				if (!quiet)
					mtd_log_write("\b\b\b   ");
				if (quiet < 2)
					mtd_log_write("Appending jffs2 data from %s to %s...\n", jffs2file, mtd);
		...
		if(!no_erase)
		{
			while (w + buflen > e) {
				...
				if (mtd_erase_block(fd, e) < 0) {
					if (next) {
						if (w < e) {
							if (swrite(fd, buf + offset, e - w) < 0) {
								mtd_log_write("Failed to write mtd : %s.\n", strerror(errno));
								mtd_log_exit();
								exit(1);
							}
							offset = e - w;
						...
						mtd_log_write("Failed to erase block : %s.\n", strerror(errno));
						mtd_log_exit();
						exit(1);
					}
				}
		...
		if ((result = swrite(fd, buf + offset, buflen)) < buflen) {
			if (result < 0) {
				mtd_log_write("Error writing image : %s.\n", strerror(errno));
				mtd_log_exit();
				exit(1);
			} else {
				mtd_log_write("Insufficient space.\n");
				mtd_log_exit();
				exit(1);
			}
		}
	...
	if (fis_layout) {
		if (fis_remap(old_parts, n_old, new_parts, n_new) < 0)
			mtd_log_write("Failed to update the FIS partition table.\n");
	}
#endif
...
{
	int ch, i, boot, imagefd = 0, force, unlocked, rwflag;
	int log_enable = 0;
	...
	if ((strcmp(argv[0], "unlock") == 0) && (argc == 2)) {
		...
		cmd = CMD_FIXSEAMA;
		device = argv[1];
	} else if ((strcmp(argv[0], "setrw") == 0) && (argc == 3)) {
		cmd = CMD_SETRWFLAG;
		device = argv[1];
		rwflag = atoi(argv[2]);
	...
	if (log_enable && device) {
		mtd_log_init(device);
		mtd_log_write("Begin log %s mtd device.\n", device);
	}
	...
	switch (cmd) {
		case CMD_UNLOCK:
			...
			break;
		case CMD_SETRWFLAG:
			mtd_setrwflag(device, rwflag);
			break;
		case CMD_VERIFY:
			mtd_verify(imagefd, device);
			break;
		case CMD_ERASE:
			if (!unlocked)
				mtd_unlock(device);
	...
	if (log_enable && device) {
		mtd_log_write("End log %s mtd device.\n", device);
		mtd_log_exit();
	}
